	...
	return (
		<div
			...
			<div
				...
				style={{ display: p.action ? "flex" : undefined, gap: p.action ? '1em': undefined }}>
				{ViewUtils.wrapper(
					p.children,
					(x) => (
						<>
							<div style={{ flexGrow: 1 }}>{x}</div>
							<div style={{ width: p.actionWidth, minWidth: p.actionWidth }}>{p.action}</div>
						</>
					),
					!!p.action
				)}
